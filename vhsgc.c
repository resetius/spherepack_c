/* vhsgc.f -- translated by f2c (version 20061008).
...
/*     work  a double precision work space that does not need to be saved */
...
/* Computing MAX */
    ...
    jw1 = lwzvin + 1;
    jw2 = jw1 + lwzvin;
    vhsgc1_(nlat, nlon, ityp, nt, &imid, idvw, jdvw, &v[v_offset], &w[
	    w_offset], mdab, ndab, &br[br_offset], &bi[bi_offset], &cr[
	    cr_offset], &ci[ci_offset], &idv, &work[1], &work[iw1], &work[iw2]
	    , &work[iw3], &work[iw4], &work[iw5], &wvhsgc[1], &wvhsgc[jw1], &
	    wvhsgc[jw2]);
    return 0;
} /* vhsgc_ */
...
/* Subroutine */ int vhsgc1_(integer *nlat, integer *nlon, integer *ityp, 
	...
	vb, real *wb, real *wvbin, real *wwbin, real *wrfft)
{
    /* System generated locals */
    integer v_dim1, v_dim2, v_offset, w_dim1, w_dim2, w_offset, br_dim1, 
	    br_dim2, br_offset, bi_dim1, bi_dim2, bi_offset, cr_dim1, cr_dim2,
...
/* Subroutine */ int vhsgci_(integer *nlat, integer *nlon, real *wvhsgc, 
...
/*     jw2 = jw1+nlat+nlat */
/*     jw3 = jw2+nlat+nlat */
    gaqd_(nlat, &dwork[jw1], &dwork[jw2], &dwork[jw3], ldwork, ierror);
/*     iwrk = nlat+2 */
    iwrk = (*nlat + 1) / 2 + 1;
    vbgint_(nlat, nlon, &dwork[1], &wvhsgc[1], &dwork[iwrk]);
    lwvbin = lzz1 + labc;
    iw1 = lwvbin + 1;
    wbgint_(nlat, nlon, &dwork[1], &wvhsgc[iw1], &dwork[iwrk]);
    iw2 = iw1 + lwvbin;
    hrffti_(nlon, &wvhsgc[iw2]);
    return 0;
} /* vhsgci_ */
