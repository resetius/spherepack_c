/* isfvpes.f -- translated by f2c (version 20061008).
...
/* Subroutine */ int isfvpes_(integer *nlat, integer *nlon, integer *isym, 
	...
	wvhses, integer *lvhses, real *work, integer *lwork, integer *ierror)
{
    ...
    /* Local variables */
    extern /* Subroutine */ int isfvpes1_(integer *, integer *, integer *, 
	    integer *, real *, real *, integer *, integer *, real *, real *, 
	    real *, real *, integer *, integer *, real *, real *, real *, 
	    real *, integer *, real *, real *, integer *, real *, integer *, 
	    integer *);
    static integer l1, l2, mn, is, ibi, ici, ibr, icr, iwk, lwk;
...
/* Computing MIN */
    i__1 = *nlat, i__2 = (*nlon + 2) / 2;
    l1 = min(i__1,i__2);
    if (*lvhses < l1 * l2 * (*nlat + *nlat - l1 + 1) + *nlon + 15) {
	return 0;
    }
    *ierror = 10;
    if (*isym == 0) {
	if (*lwork < *nlat * (((*nt << 1) + 1) * *nlon + (l1 << 2) * *nt + 1))
		 {
	    return 0;
	}
    } else {
	if (*lwork < ((*nt << 1) + 1) * *nlon + *nlat * ((l1 << 2) * *nt + 1))
		 {
	    return 0;
	}
    }
...
/* Subroutine */ int isfvpes1_(integer *nlat, integer *nlon, integer *isym, 
	...
	real *bi, real *cr, real *ci, integer *mab, real *fnn, real *wvhses, 
	integer *lvhses, real *wk, integer *lwk, integer *ierror)
{
    /* System generated locals */
    integer v_dim1, v_dim2, v_offset, w_dim1, w_dim2, w_offset, as_dim1, 
	    as_dim2, as_offset, bs_dim1, bs_dim2, bs_offset, av_dim1, av_dim2,
...
/*     synthesize br,bi,cr,ci into (v,w) */
    ...
    vhses_(nlat, nlon, &ityp, nt, &v[v_offset], &w[w_offset], idv, jdv, &br[
	    br_offset], &bi[bi_offset], &cr[cr_offset], &ci[ci_offset], mab, 
	    nlat, &wvhses[1], lvhses, &wk[1], lwk, ierror);
    return 0;
} /* isfvpes1_ */
