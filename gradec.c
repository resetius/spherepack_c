/* gradec.f -- translated by f2c (version 20061008).
...
/* Subroutine */ int gradec_(integer *nlat, integer *nlon, integer *isym, 
	...
	doublereal *wvhsec, integer *lvhsec, doublereal *work, integer *lwork,
	 integer *ierror)
{
...
/* Computing MAX */
	i__1 = l2 * 6;
	lwkmin = *nlat * ((*nt << 1) * *nlon + max(i__1,*nlon)) + *nlat * ((
		l1 << 1) * *nt + 1);
    } else {
/* Computing MAX */
	i__1 = *nlat * 6;
...
/*     set work space pointers */
    ...
    gradec1_(nlat, nlon, isym, nt, &v[v_offset], &w[w_offset], idvw, jdvw, &
	    work[ibr], &work[ibi], &mmax, &work[is], mdab, ndab, &a[a_offset],
	     &b[b_offset], &wvhsec[1], lvhsec, &work[iwk], &liwk, ierror);
    return 0;
} /* gradec_ */
